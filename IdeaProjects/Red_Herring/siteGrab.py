...
def scrapeSite(targetURL, targetSite):
    ...
    for js in soup.find_all('script'):
        try:
            print js.get('src')
            if js.get('src')[0] == '/' and js.get('src')[1] != '/':
                ...
                jsFile.write(jsData)
                jsFile.close()
            elif js.get('src')[0] == '/' and js.get('src')[1] == '/':
                #print js.get('src')[2:]
            ...
            continue
        print os.path.splitext(os.path.basename(js.get('src'))[0])
        print os.path.splitext(os.path.basename(js.get('src'))[1])
